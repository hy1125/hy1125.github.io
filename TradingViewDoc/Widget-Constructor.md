...
查看下列完整支持的参数列表。请记住，在图表初始化后在更改这些参数是不起作用的。如果要在初始化图表之后更改图表的状态，请使用[widget方法](Widget-Methods.md)。

属性标记为![](images/trading.png)的只在交易终端可用。

#### symbol, interval
您的图表的初始商品和周期。`interval`的格式在另一篇[文章](Resolution.md)中说明。 *必须项*
...
JavaScript对象的实现接口 [JS API](JS-Api.md) 以反馈图表及数据。 *必须项*

#### timezone

图表的初始时区。时间刻度上的数字取决于这个时区。请参阅[支持的时区列表](Symbology.md#timezone)。设置为交易所时区。覆盖默认值，您应该使用[覆盖章节](Widget-Constructor.md#overrides)。
...
图表库的本地化处理 。详情：[本地化](Localization.md)
...
这个 `override`将使水印100％不透明（不可见）。 所有可定制的属性都列在[单独的文章](Overrides.md)中。 从1.5开始，您可以使用绘图覆盖。[绘图覆盖](Drawings-Overrides.md).
...
包含您编写的指标的文件路径。 查看[更多细节](Creating-Custom-Studies.md)。
...
使用此选项自定义默认指标的样式及输入值。 您还可以使用此参数自定义`Compare`数据列的样式和输入值。 [查看更多](Studies-Overrides.md)
...
时间范围是一个包含`text`和`resolution属性的对象。文本必须具有以下格式：<integer><y|m|d>`\( \d+\(y\|m\|d\) 为正则表达式 \). 周期是具有通用周期格式的字符串. 请参阅[本主题](Time-Frames.md)了解有关时间范围的更多信息。在1.7中添加了描述属性，并显示在弹出菜单中。此参数是可选的（如果时间范围描述符不包含此属性：title（如果指定）或使用）。title属性在1.9中添加，此值将覆盖从text属性生成的默认标题。 此参数是可选的。

#### charts\_storage\_url, client\_id, user\_id

这些参数是有关于高阶图表的保存/加载。 查看[更多细节](Saving-and-Loading-Charts.md).
...
默认支持该项目。此选项需要禁用localstorage的使用（请参阅[功能集](Featuresets.md)以了解更多\)。`favorites`property 为一个对象，拥有以下属性：
...
交易控制器可以让您在线交易。[阅读更多](Trading-Controller.md)
...
使用这个字段来传递构造[经纪商API](Broker-API.md)的实现函数。 
这是一个接收[交易主机](Trading-Host.md)并返回[经纪商API](Broker-API.md)的函数。

#### ![](images/trading.png) brokerConfig
`brokerConfig: { configFlags: {...} }`
使用此字段设置交易终端的配置标志。 [了解更多](Broker-API.md#tradingconfiguration)
...
* [覆盖默认指标参数](Studies-Overrides.md)
* [覆盖默认图表参数](Overrides.md)
